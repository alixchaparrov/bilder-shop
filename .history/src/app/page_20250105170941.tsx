...
export default function Home() {
  ...
  const { user, isLoading } = useAuth(); // Suponiendo que `isLoading` está disponible

  // Si la autenticación está cargando, muestra un indicador
  if (isLoading) {
    return <p className="text-center mt-12">Loading...</p>;
  }

  // Si el usuario no está autenticado, redirige o muestra una pantalla alternativa
  if (!user) {
    return <p className="text-center mt-12">Por favor, inicia sesión.</p>;
  }

  // Si el usuario es administrador, muestra el AdminDashboard
  if (user.role === "admin") {
