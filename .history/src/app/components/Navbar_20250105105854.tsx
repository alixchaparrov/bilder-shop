...
export default function Navbar({
  ...
  const [isCartOpen, setIsCartOpen] = useState(false); // Estado para mostrar el carrito
  const [isAuthModalOpen, setIsAuthModalOpen] = useState(false); // Estado para mostrar la modal de autenticación
  ...
  const handleLogout = () => {
    logout();
    alert("Sie haben sich erfolgreich abgemeldet."); // Notificación al cerrar sesión
  };
        ...
        >
          ...
          <div className="flex items-center space-x-4">
            ...
            {user ? (
              <div className="flex items-center space-x-4">
                <span>
                  Willkommen, {user.email} {user.role === "admin" && "(Admin)"}
                </span>
                {user.role === "admin" && (
                  <button
                    className="bg-purple-500 hover:bg-purple-600 text-white px-4 py-2 rounded"
                    onClick={() => console.log("Admin-Dashboard öffnen")}
                  >
                    Admin-Dashboard
                  </button>
                )}
