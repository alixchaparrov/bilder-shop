...
const AuthComponent: React.FC = () => {
  ...
  return (
    <div className="auth-container">
      <h2>{state.isLogin ? 'Anmelden' : 'Registrieren'}</h2>
      <form>
        <div>
          <label>Email</label>
          <input
        ...
        <button type="button" onClick={state.isLogin ? handleLogin : handleSignup}>
          {state.isLogin ? 'Anmelden' : 'Registrieren'}
        </button>
      </form>
      <p onClick={toggleLoginSignup} style={{ cursor: 'pointer' }}>
        {state.isLogin ? 'Haben Sie noch kein Konto? Registrieren' : 'Sie haben bereits ein Konto? Loggen Sie sich in Ihr Konto ein'}
      </p>
    </div>
  );
};
