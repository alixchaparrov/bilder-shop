        ...
        {cart.length > 0 ? (
          <>
              ...
              {cart.map((item) => (
                <li key={item._id} className="flex justify-between items-center border-b pb-2">
                  ...
                  <div className="flex items-center space-x-2">
                    <button onClick={() => removeOneFromCart(item._id)} className="text-yellow-500 hover:text-yellow-700">
                      <FaMinus />
                    </button>
                    <button onClick={() => addToCart(item)} className="text-green-500 hover:text-green-700">
                      <FaPlus />
                    </button>
            ...
            <div className="mt-6 flex justify-between items-center">
              ...
              <span className="text-lg font-bold">Gesamtpreis: {getTotalPrice().toFixed(2)} €</span>
            </div>
          </>
        ) : (
